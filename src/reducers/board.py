...
def board(state=None, action=None):
	...
	if ty == 'REMOVE_COLOR':
		...
		cities_to_remove = [ city for city in map_colors if map_colors[city] == action.get('color') ]
		new_state = { key: state[key] for key in state if key not in cities_to_remove }
		return new_state
