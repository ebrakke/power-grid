...
def get_available_bucket(state, resource_type):
    ...
    buckets_with_resources = [i for (i, bucket) in enumerate(
        state) if bucket.get(resource_type) > 0]
...
def get_first_non_filled_bucket(state, resource_type):
    ...
    non_full_buckets = [i for (i, bucket) in enumerate(state) if bucket.get(
        resource_type) < MAX_RESOURCES[i][resource_type]]
    return non_full_buckets[-1] if non_full_buckets else -1


def resources(state=None, action=None):
    if state is None:
        state = deepcopy(INITIAL_RESOURCES)
    if action is None:
        return state
    ...
    if action_type == 'TAKE_RESOURCE':
        ...
        new_state[idx][action.get('resource_type')] -= 1
        return new_state
