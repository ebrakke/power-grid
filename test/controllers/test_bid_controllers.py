"""Tests for the bid controller"""
import unittest
from copy import deepcopy
from src.controllers.bid import bid, valid_bid, bid_is_won, get_next_player
...
class TestBidControllers(unittest.TestCase):
    ...
    def test_when_given_a_valid_bid_it_should_return_true(self):
        self.assertTrue(valid_bid('player_1', 20, self.card_6,
                                  self.base_game_state, self.players, self.market))

    def test_fail_when_bidding_on_different_card(self):
        self.assertFalse(valid_bid('player_1', 20, self.card_10,
                                   self.base_game_state, self.players, self.market))

    def test_fail_when_bidding_when_passed(self):
        self.assertFalse(valid_bid('player_3', 20, self.card_6,
                                   self.base_game_state, self.players, self.market))

    def test_false_when_bidding_more_than_you_have(self):
        self.assertFalse(valid_bid('player_1', 51, self.card_6,
                                   self.base_game_state, self.players, self.market))

    def test_false_when_you_have_too_many_power_plants(self):
        test_players = deepcopy(self.players)
        test_players[0]['power_plants'] = [
            self.card_42, self.card_10, self.card_18, self.card_3]
        self.assertFalse(valid_bid('player_1', 20, self.card_6,
                                   self.base_game_state, test_players, self.market))
    ...
    def test_next_player_after_bid_is_player_1(self):
        test_state = deepcopy(self.base_game_state)
        test_state['current_bid'] = dict(
            player_id='', card=None, amount=0, passed=[])
    ...
    def test_next_player_after_player_1_bid_is_player_2(self):
        test_state = deepcopy(self.base_game_state)
        test_state['current_bid'] = dict(
            player_id='player_1', card=self.card_6, amount=20, passed=['player_3'])
    ...
    def test_next_player_is_one_with_4_powerplants(self):
        ...
        self.assertEqual(self.players[0].get('player_id'), get_next_player(
            self.base_game_state, test_players))

    def test_next_player_is_last_ranked_in_next_phase(self):
        test_state = deepcopy(self.base_game_state)
        test_state['phase'] = 3
        test_state['bought_or_passed'] = []
        self.assertEqual(self.base_game_state['player_rank'][3], get_next_player(
            test_state, self.players))

    def test_next_player_is_next_after_initial_bid(self):
        test_state = deepcopy(self.base_game_state)
        test_state['bought_or_passed'] = []
        test_state['current_bid'] = dict(
            player_id=None, amount=0, card=None, passed=[])
