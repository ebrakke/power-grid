...
class TestGameReducer(unittest.TestCase):
    ...
    def test_updates_current_bid_when_no_bid_exists(self):
        state = game()
        next_state = game(state, dict(type='BID_ON_POWER_PLANT',
                                      card={'market_cost': 3}, amount=3))
        self.assertEqual(next_state['current_bid']['amount'], 3)

    def test_does_not_update_current_bid_when_bid_is_higher_than_amount(self):
        state = game(dict(turn=0, phase=0, current_bid={
                     'player_id': 1, 'card': {}, 'amount': 6}))
        next_state = game(state, dict(
            type='BID_ON_POWER_PLANT', card={}, amount=4))
        self.assertEqual(next_state['current_bid']['amount'], 6)

    def test_adds_player_to_player_order(self):
        state = game()
        next_state = game(state, dict(
            type='CREATE_PLAYER', player_id='abc123'))
